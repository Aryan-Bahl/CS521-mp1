...
__global__ void gemm_gpu_o1_kernel(float* __restrict__ A, float* __restrict__ B, float * __restrict__ C, int M, int N, int K) {
...
{
    // 2D grid of 16x16 threads, each thread computes one C[i,j]
    dim3 block(16, 16);
...
#define TILE_O2 16
__global__ void gemm_gpu_o2_kernel(float* __restrict__ A, float* __restrict__ B, float * __restrict__ C, int M, int N, int K) {
    __shared__ float As[TILE_O2][TILE_O2];
    __shared__ float Bs[TILE_O2][TILE_O2];

    int row = blockIdx.y * TILE_O2 + threadIdx.y;
    int col = blockIdx.x * TILE_O2 + threadIdx.x;

    float acc = 0.0f;
    int numTiles = (K + TILE_O2 - 1) / TILE_O2;

    for (int t = 0; t < numTiles; ++t) {
        int aCol = t * TILE_O2 + threadIdx.x;
        int bRow = t * TILE_O2 + threadIdx.y;
...
{
    dim3 block(TILE_O2, TILE_O2);
    dim3 grid((N + TILE_O2 - 1) / TILE_O2, (M + TILE_O2 - 1) / TILE_O2);
    gemm_gpu_o2_kernel<<<grid, block>>>(A, B, C, M, N, K);
}

#define TILE_O3 32
__global__ void gemm_gpu_o3_kernel(float* __restrict__ A, float* __restrict__ B, float * __restrict__ C, int M, int N, int K) {
    __shared__ float As[TILE_O3][TILE_O3];
    __shared__ float Bs[TILE_O3][TILE_O3];

    int row = blockIdx.y * TILE_O3 + threadIdx.y;
    int col = blockIdx.x * TILE_O3 + threadIdx.x;

    float acc = 0.0f;
    int numTiles = (K + TILE_O3 - 1) / TILE_O3;

    for (int t = 0; t < numTiles; ++t) {
        int aCol = t * TILE_O3 + threadIdx.x;
        int bRow = t * TILE_O3 + threadIdx.y;
...
{
    dim3 block(TILE_O3, TILE_O3);
    dim3 grid((N + TILE_O3 - 1) / TILE_O3, (M + TILE_O3 - 1) / TILE_O3);
